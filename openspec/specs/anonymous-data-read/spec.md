# anonymous-data-read Specification
...
Access to dedicated anonymous data endpoints SHALL require `can_access_protected` permission. Requests without sufficient permission SHALL receive 401 (if unauthenticated) or 403 (if authenticated but unauthorized).

#### Scenario: Unauthenticated request to anonymous data endpoint
- **WHEN** an unauthenticated request is made to `GET /api/v2/{owner}/datasets/{ds}/places/anonymous`
- **THEN** the system SHALL respond with 401 Unauthorized

#### Scenario: Authenticated but unauthorized request to anonymous data endpoint
- **WHEN** an authenticated user without `can_access_protected` permission requests `GET /api/v2/{owner}/datasets/{ds}/places/anonymous`
- **THEN** the system SHALL respond with 403 Forbidden

#### Scenario: Dataset owner accesses anonymous data endpoint
- **WHEN** the dataset owner makes a request to `GET /api/v2/{owner}/datasets/{ds}/places/anonymous`
- **THEN** the system SHALL respond with 200 OK and the paginated results
