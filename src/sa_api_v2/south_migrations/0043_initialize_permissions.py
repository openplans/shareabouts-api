...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # By default, all datasets are open for reading.
        for ds in orm.DataSet.objects.all():
            if not ds.permissions.exists():
                ds.permissions.add(orm.DataSetPermission(submission_set='*', priority=0, can_retrieve=True))

        # By default, API keys and origins provide full access.
        for key in orm['apikey.ApiKey'].objects.all():
            if not key.permissions.exists():
                key.permissions.add(orm.KeyPermission(submission_set='*', priority=0, can_retrieve=True, can_update=True, can_create=True, can_destroy=True))
        for origin in orm['cors.Origin'].objects.all():
            if not origin.permissions.exists():
                origin.permissions.add(orm.OriginPermission(submission_set='*', priority=0, can_retrieve=True, can_update=True, can_create=True, can_destroy=True))
