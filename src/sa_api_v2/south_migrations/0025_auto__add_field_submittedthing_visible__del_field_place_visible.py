...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'SubmittedThing.visible'
        db.add_column('sa_api_submittedthing', 'visible',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)

        # Manually migrate the data from the Place to the Submitted Thing
        sql = """
        UPDATE sa_api_submittedthing AS st
        SET visible = p.visible
        FROM sa_api_place AS p
        WHERE p.submittedthing_ptr_id = st.id
        """
        db.execute(sql)

        # Deleting field 'Place.visible'
        db.delete_column('sa_api_place', 'visible')

    def backwards(self, orm):
        # Adding field 'Place.visible'
        db.add_column('sa_api_place', 'visible',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)

        # Manually migrate the data from the Submitted Thing to the Place
        sql = """
        UPDATE sa_api_place AS p
        SET visible = st.visible
        FROM sa_api_submittedthing AS st
        WHERE p.submittedthing_ptr_id = st.id
        """
        db.execute(sql)

        # Deleting field 'SubmittedThing.visible'
        db.delete_column('sa_api_submittedthing', 'visible')
    ...
    models = {
        ...
        'sa_api_v2.submittedthing': {
            ...
            'dataset': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'submitted_thing_set'", 'blank': 'True', 'to': "orm['sa_api_v2.DataSet']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
